...
if($_POST)
{
    ...
    try
    {
        ...
        if($count==0){
            $stmt = $db_con->prepare("UPDATE tbl_users SET poi_name = :pname, group_id = :grid, poi_kana =:pkana,  joining_date =:jdate, poi_lat=:plat, poi_lon =:plon, icon_lat =:ilat, icon_lon=:ilon,  point_type=:ptype, next_voice_id=:nvid, voice_id=:vid, admin_id=:adid, ev_flag=:eflag, tourist_detail=:tode,business_hours=:busho,parking=:park,closing_day=:cloday,pay=:pay,credit_card=:crecard ");
